                    ...
                    <div class="row mt-4">
                        <div class="col-12 col-md-6 mx-auto">
                            <div class="card p-3">
                                ...
                                <a href="#" id="checkout-btn" class="btn btn-success btn-lg w-100 checkout-btn mt-2"><i class="fas fa-shopping-cart me-2"></i>Proceed to Checkout</a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (checkoutBtn) {
            checkoutBtn.addEventListener('click', function(e) {
                ...
                var items = [];
                <?php
                // Prepare a JS array of product names in the cart
                if (!empty($_SESSION['cart']) && is_array($_SESSION['cart'])) {
                    foreach ($_SESSION['cart'] as $product_id => $qty) {
                        if ($qty < 1) continue;
                        ...
                        $product = $stmt->fetch(PDO::FETCH_ASSOC);
                        if ($product) {
                            echo "items.push('" . addslashes($product['name']) . " (x$qty)');\n";
                        }
                    }
                }
                ?>
                if (items.length > 0) {
                    alert('You bought these items: ' + items.join(', '));
                    // After confirmation, redirect to clear_cart.php
